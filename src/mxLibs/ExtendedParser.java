...
import static mxLibs.ResearchObjectives.defaultValue;

public class ExtendedParser extends ContentParser {
    ...
    public ContentParser pa = new ContentParser();
    ...
    ObjectMap<Class<?>, FieldParser> classParsers = new ObjectMap<>() {{
        ...
        put(Schematic.class, (type, data) -> {
            ...
            if (result != null) {
                ...
                if (str.startsWith(schematicBaseStart)) {
                    return Schematics.readBase64(str);
                } else {
                    return Schematics.read(tree.get("schematics/" + str + "." + schematicExtension));
                }
            }
        });
    ...
    private <T extends Content> T find(ContentType type, String name) {
        Content c = content.getByName(type, name);
        if (c == null) c = content.getByName(type, currentMod.name + "-" + name);
    ...
    private void init() {
        for (ContentType type : ContentType.all) {
            Seq<Content> arr = content.getBy(type);
            if (!arr.isEmpty()) {
    ...
    public JsonValue parserJson(String s) {
        return parser.fromJson(null, s);
    }
        ...
        private <T> T internalRead(Class<T> type, Class elementType, JsonValue jsonData, Class keyType) {
            if (type != null) {
                ...
                if (Content.class.isAssignableFrom(type)) {
                    ...
                    T one = (T) content.getByName(ctype, prefix + jsonData.asString());
                    if (one != null) return one;
                    T two = (T) content.getByName(ctype, jsonData.asString());
    ...
    private ObjectMap<ContentType, TypeParser<?>> baseParsers = ObjectMap.of(
            ContentType.block, (TypeParser<Block>) (mod, name, value) -> {
                ...
                read(() -> {
                    ...
                    JsonValue v = null;
                    if (value.has("researchObjectives") && value.get("researchObjectives").isArray()) {
                        v = value.get("researchObjectives");
                        value.remove("researchObjectives");
                    }
                    defaultValue.put(block, value);
                    ...
                    if (v != null) ResearchObjectives.toBeParsed.put(block, v);
    ...
    private ObjectMap<ContentType, TypeParser<?>> parsers = ObjectMap.of(
            ContentType.block, (TypeParser<Block>) (mod, name, value) -> {
                ...
                if (locate(ContentType.block, name) != null) {
                    ...
                    block = make(resolve(value.getString("extendedType", ""), Block.class), mod + "-" + name);
                }

                if (value.has("extendedType")) value.remove("extendedType");
                ...
                read(() -> {
                    ...
                    if (value.has("craftPlans") && value.get("craftPlans").isArray()) {
                        for (JsonValue child : value.get("craftPlans")) {


                            Seq<Consume> consumeBuilder = new Seq<>();
                            for (JsonValue c : child.get("consumes")) {
                                switch (c.name) {
                                    case "item" ->
                                            consumeBuilder.add(new ConsumeItems(new ItemStack[]{new ItemStack(find(ContentType.item, child.asString()), 1)}));
                                    case "itemCharged" ->
                                            consumeBuilder.add(parser.readValue(ConsumeItemCharged.class, child));
                                    case "itemFlammable" ->
                                            consumeBuilder.add(parser.readValue(ConsumeItemFlammable.class, child));
                                    case "itemRadioactive" ->
                                            consumeBuilder.add(parser.readValue(ConsumeItemRadioactive.class, child));
                                    case "itemExplosive" ->
                                            consumeBuilder.add(parser.readValue(ConsumeItemExplosive.class, child));
                                    case "itemExplode" ->
                                            consumeBuilder.add(parser.readValue(ConsumeItemExplode.class, child));
                                    case "items" -> consumeBuilder.add(c.isArray() ?
                                            new ConsumeItems(parser.readValue(ItemStack[].class, c)) :
                                            parser.readValue(ConsumeItems.class, c));
                                    case "liquidFlammable" ->
                                            consumeBuilder.add(parser.readValue(ConsumeLiquidFlammable.class, child));
                                    case "liquid" -> consumeBuilder.add(parser.readValue(ConsumeLiquid.class, child));
                                    case "liquids" -> consumeBuilder.add(c.isArray() ?
                                            new ConsumeLiquids(parser.readValue(LiquidStack[].class, c)) :
                                            parser.readValue(ConsumeLiquids.class, c));
                                    case "coolant" -> consumeBuilder.add(parser.readValue(ConsumeCoolant.class, child));
                                    case "power" -> {
                                        throw new IllegalArgumentException("Consume power should not be used in craft plans. Maybe you can use consumes like this to set global consumption:\n consumes:{\n    power:60\n}");
                                    }
                                    default ->
                                            throw new IllegalArgumentException("Unknown consumption type: '" + c.name + "' for block '" + block.name + "'.");
                                }
                            }
                            MultiCrafter.CraftPlan craftPlan = new MultiCrafter.CraftPlan(consumeBuilder);
                            if (child.has("outputItem") && child.get("outputItem").isString()) {
                                craftPlan.outputItem = parser.readValue(ItemStack.class, child.get("outputItem"));
                            }
                            ...
                            if (child.has("outputLiquid") && child.get("outputLiquid").isString()) {
                                craftPlan.outputLiquid = parser.readValue(LiquidStack.class, child.get("outputLiquid"));
                            }
                            ...
                            if (block instanceof MultiCrafter multiCrafter) {
                                multiCrafter.craftPlans.add(craftPlan);
                            }
                        }
            ...
            ContentType.unit, (TypeParser<UnitType>) (mod, name, value) -> {
                ...
                if (locate(ContentType.unit, name) == null) {
                    ...
                    var typeVal = value.get("extendedType");
                    if (unit.constructor == null || typeVal != null) {
                        if (typeVal != null && !typeVal.isString()) {
                            throw new RuntimeException("Unit '" + name + "' has an incorrect type. Types must be strings.");
                        }
    ...
    public Content parse(LoadedMod mod, String name, String json, Fi file, ContentType type) throws Exception {
        ...
        Content c;
        if (value.has("extendedType")) {
            if (value.has("type"))
                throw new IllegalArgumentException("An object can not have [type] and [extendedType] at the same time ");
            boolean located = locate(type, name) != null;
            c = parsers.get(type).parse(mod.name, name, value);
            ...
            boolean located = locate(type, name) != null;
            c = baseParsers.get(type).parse(mod.name, name, value);
            c.minfo.sourceFile = file;
            toBeParsed.add(c);
            if (!located) {
                c.minfo.mod = mod;
    ...
    private <T extends MappableContent> T locate(ContentType type, String name) {
        T first = content.getByName(type, name); //try vanilla replacement
        return first != null ? first : content.getByName(type, currentMod.name + "-" + name);
    }
    ...
    private PartProgress parseProgressOp(PartProgress base, String op, JsonValue data) {
        ...
        return switch (op) {
            ...
            case "add" ->
                    data.has("amount") ? base.add(data.getFloat("amount")) : base.add(parser.readValue(PartProgress.class, data.get("other")));
            case "blend" ->
                    base.blend(parser.readValue(PartProgress.class, data.get("other")), data.getFloat("amount"));
            case "mul" ->
                    data.has("amount") ? base.mul(data.getFloat("amount")) : base.mul(parser.readValue(PartProgress.class, data.get("other")));
            ...
            case "curve" ->
                    data.has("interp") ? base.curve(parser.readValue(Interp.class, data.get("interp"))) : base.curve(data.getFloat("offset"), data.getFloat("duration"));
            default ->
    ...
    public void replaceBlock(Object object, JsonValue jsonMap) {
